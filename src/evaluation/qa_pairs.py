...
QA_PAIRS: list[dict[str, Any]] = [
    ...
    {"id": 8,  "level": 2, "question": "What is the query latency of IndexFlatIP vs IndexIVF?", "expected": "IndexFlatIP: 4 ms, IndexIVF: 1 ms", "source": "rag_design_notes.md", "keywords": [["4 ms", "4ms", "4 millisecond"], ["1 ms", "1ms", "1 millisecond"]]},
    ...
    {"id": 16, "level": 4, "question": "What two pre-training tasks does BERT use?", "expected": "Masked Language Modeling (MLM) and Next Sentence Prediction (NSP)", "source": "bert_devlin2018.pdf", "keywords": [["masked language", "masked lm", "mlm"], ["next sentence", "nsp"]]},
    ...
    {"id": 18, "level": 4, "question": "What type of masking does REALM use during pre-training?", "expected": "Salient span masking: it masks named entities and dates rather than random tokens.", "source": "realm_guu2020.pdf", "keywords": ["salient span"]},
    ...
    {"id": 21, "level": 5, "question": "What is the training cost of the base Transformer model in floating point operations (FLOPs)?", "expected": "3.3 x 10^18 FLOPs", "source": "attention_is_all_you_need.pdf", "keywords": ["3.3"]},
    ...
    {"id": 23, "level": 5, "question": "What is REALM's best Exact Match score on NaturalQuestions Open (the full CC-News model)?", "expected": "40.4 EM", "source": "realm_guu2020.pdf", "keywords": ["40.4"]},
    ...
    {"id": 25, "level": 6, "question": "In the original Transformer of Vaswani et al. 2017 (Attention Is All You Need), how many attention heads and what model dimension (d_model) does the base model use?", "expected": "8 heads, d_model = 512", "source": "attention_is_all_you_need.pdf", "keywords": ["512"]},
    {"id": 26, "level": 6, "question": "What is the feed-forward inner-layer dimensionality (d_ff) of the base Transformer in Vaswani et al. 2017?", "expected": "d_ff = 2048", "source": "attention_is_all_you_need.pdf", "keywords": ["2048"]},
    ...
    {"id": 28, "level": 7, "question": "What scaling factor does scaled dot-product attention apply before the softmax?", "expected": "1/sqrt(d_k): divide by the square root of the key dimension", "source": "attention_is_all_you_need.pdf", "keywords": [["square root", "sqrt", "√"]]},
    ...
    {"id": 32, "level": 8, "question": "How does RAG-Sequence (Lewis 2020) compare to REALM (Guu 2020) on WebQuestions (WQ) Open-Domain QA Exact Match?", "expected": "RAG-Sequence achieves 45.2 EM vs REALM's 40.7 EM; RAG outperforms REALM by 4.5 points on WebQuestions.", "source": "rag_lewis2020.pdf + realm_guu2020.pdf", "keywords": ["45.2", "40.7"]},
]
